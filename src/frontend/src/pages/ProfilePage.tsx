...
export default function ProfilePage({ userId }: { userId: number }) {
  ...
  // Editing state: tracks which item is being edited — { type, id }
    ...
    <div className="page">
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1rem" }}>
        ...
        <label className="btn-autofill" style={{ cursor: filling ? "wait" : "pointer" }}>
          {filling ? "Extracting from CV..." : "Upload CV (PDF) to auto-fill"}
      ...
      <Section title="AI Instructions">
        <p className="hint" style={{ margin: "0 0 8px" }}>
          These instructions are sent to the AI for every generation (CV adaptation, cover letter, skill gap).
          For example: &quot;Do not change the Education section&quot;, &quot;Always keep Python first in skills&quot;.
        </p>
        <textarea
          value={aiInstructions}
          onChange={(e) => setAiInstructions(e.target.value)}
          placeholder="e.g.&#10;- Do not modify the Education section&#10;- Keep all experiences, do not remove any&#10;- Always mention Python and C++ first in skills"
          rows={5}
          style={{ width: "100%", fontFamily: "inherit", fontSize: "14px", padding: "8px", boxSizing: "border-box" }}
        />
        <div style={{ display: "flex", gap: "0.5rem", marginTop: "8px", alignItems: "center" }}>
          <button onClick={handleSaveInstructions} disabled={!instructionsChanged || savingInstructions}>
      ...
      <Section title="Skills">
        <form onSubmit={handleAddSkill} className="inline-form">
      ...
      <Section title="Experiences">
        <form onSubmit={handleAddExp} className="inline-form" style={{ flexWrap: "wrap" }}>
          ...
          <input type="month" placeholder="Start" value={expStart} onChange={(e) => setExpStart(e.target.value)} title="Start date" />
          <input type="month" placeholder="End" value={expEnd} onChange={(e) => setExpEnd(e.target.value)} title="End date" />
          <input placeholder="Description" value={expDesc} onChange={(e) => setExpDesc(e.target.value)} style={{ flex: "1 1 100%" }} />
          <button type="submit">Add</button>
        </form>
        <ul className="item-list">
          {experiences.map((exp) =>
            isEditing("exp", exp.id) ? (
              <li key={exp.id} className="editing-row" style={{ flexWrap: "wrap" }}>
                ...
                <input type="month" value={editField("start_date")} onChange={(e) => setEditField("start_date", e.target.value)} title="Start date" />
                <input type="month" value={editField("end_date")} onChange={(e) => setEditField("end_date", e.target.value)} title="End date" />
              ...
              <li key={exp.id}>
                ...
                {(exp.start_date || exp.end_date) && (
                  <span className="date-range"> ({exp.start_date ?? "?"} - {exp.end_date ?? "present"})</span>
                )}
      ...
      <Section title="Education">
        <form onSubmit={handleAddEdu} className="inline-form" style={{ flexWrap: "wrap" }}>
          ...
          <input placeholder="Description" value={eduDesc} onChange={(e) => setEduDesc(e.target.value)} style={{ flex: "1 1 100%" }} />
          <button type="submit">Add</button>
        </form>
        <ul className="item-list">
          {education.map((ed) =>
            isEditing("edu", ed.id) ? (
              ...
              <li key={ed.id}>
                ...
                {(ed.start_date || ed.end_date) && (
                  <span className="date-range"> ({ed.start_date ?? "?"} - {ed.end_date ?? "present"})</span>
                )}
                {ed.description && <p className="item-desc">{ed.description}</p>}
      ...
      <Section title="Languages">
        <form onSubmit={handleAddLang} className="inline-form">
          <input placeholder="Language" value={langName} onChange={(e) => setLangName(e.target.value)} />
          <select value={langLevel} onChange={(e) => setLangLevel(e.target.value)}>
      ...
      <Section title="Extracurriculars">
        <form onSubmit={handleAddExtra} className="inline-form">
          <input placeholder="Name" value={extraName} onChange={(e) => setExtraName(e.target.value)} />
          <input placeholder="Description (optional)" value={extraDesc} onChange={(e) => setExtraDesc(e.target.value)} />
          <button type="submit">Add</button>
        </form>
        <ul className="item-list">
          {extras.map((ex) =>
            isEditing("extra", ex.id) ? (
              ...
              <li key={ex.id}>
                ...
                {ex.description && <p className="item-desc">{ex.description}</p>}
